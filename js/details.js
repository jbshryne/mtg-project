...
$(function () {
  ...
  if (currentPage === "details" || currentPage === "random") {
    ...
    function replaceSymbolsWithImages(oracleText) {
      // Regular expression to match symbols like {T}, {W}, etc.
      const symbolRegex = /\{([^}]+)\}/g;
    
      // Replace each matched symbol with its corresponding image
      const replacedText = oracleText.replace(symbolRegex, (match, symbol) => {
        const foundSymbol = symbolData.data.find(data => data.symbol === `{${symbol}}`);
        if (foundSymbol) {
    ...
    const manaCostWithImages = replaceSymbolsWithImages(card.mana_cost)
    ...
    if (card.mana_cost !== "") {
      ...
      $(`<td class="stat-value">${manaCostWithImages}</td>`).appendTo($manaValueRow)
    }
    ...
    $(`<td class="stat-value">${textWithImages}</td>`).appendTo($rulesTextRow)
  }

  if (card.power && card.toughness) {
    const $powerToughnessRow = $('.power-toughness')
    $(`<td class="stat-label">Power / Toughness:</td>`).appendTo($powerToughnessRow)
    $(`<td class="stat-value">${card.power} / ${card.toughness}</td>`).appendTo($powerToughnessRow)
